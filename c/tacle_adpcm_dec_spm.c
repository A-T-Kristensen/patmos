...
void adpcm_dec_decode(volatile _SPM int* input);
int adpcm_dec_filtez(volatile _SPM int *bpl, volatile _SPM int *dlt);
void adpcm_dec_upzero(int dlt, volatile _SPM int *dlti, volatile _SPM int *bli);
...
struct adpcm_struct {
	...
	int test_data[2*TEST_SIZE];
};

volatile _SPM struct adpcm_struct *adpcm_data = (volatile _SPM struct adpcm_struct *) SPM_BASE;

const unsigned int ADDR_H		= 3*TEST_SIZE*sizeof(int);
const unsigned int ADDR_ACCUMC 	= ADDR_H + sizeof(adpcm_dec_h);
const unsigned int ADDR_ACCUMD 	= ADDR_ACCUMC + sizeof(adpcm_dec_accumc);
const unsigned int ADDR_BPH 	= ADDR_ACCUMD + sizeof(adpcm_dec_accumd);
const unsigned int ADDR_DHX 	= ADDR_BPH + sizeof(adpcm_dec_dec_del_bph);
const unsigned int ADDR_BPL 	= ADDR_DHX + sizeof(adpcm_dec_dec_del_dhx);
const unsigned int ADDR_DLTX 	= ADDR_BPL + sizeof(adpcm_dec_dec_del_bpl);
...
volatile _SPM int *adpcm_dec_dec_del_bph_p	= (volatile _SPM int *) ADDR_BPH;
volatile _SPM int *adpcm_dec_dec_del_dhx_p	= (volatile _SPM int *) ADDR_DHX;
volatile _SPM int *adpcm_dec_dec_del_bpl_p	= (volatile _SPM int *) ADDR_BPL;
...
void adpcm_dec_decode(volatile _SPM int* input)
{
	...
	volatile _SPM int *h_ptr, *ac_ptr, *ac_ptr1, *ad_ptr, *ad_ptr1;
	...
	adpcm_dec_dec_szl = adpcm_dec_filtez(adpcm_dec_dec_del_bpl_p,
										 adpcm_dec_dec_del_dltx_p);
	...
	adpcm_dec_upzero(adpcm_dec_dec_dlt, adpcm_dec_dec_del_dltx_p,
					 adpcm_dec_dec_del_bpl_p);
	...
	/* filtez: compute predictor output for zero section */
	adpcm_dec_dec_szh = adpcm_dec_filtez(adpcm_dec_dec_del_bph_p,
										 adpcm_dec_dec_del_dhx_p);
	...
	/* upzero: update zero section predictor coefficients */
	adpcm_dec_upzero(adpcm_dec_dec_dh, adpcm_dec_dec_del_dhx_p,
					 adpcm_dec_dec_del_bph_p);
	...
	h_ptr = adpcm_dec_h_p;
	ac_ptr = adpcm_dec_accumc_p;
	ad_ptr = adpcm_dec_accumd_p;
...
void adpcm_dec_init()
{
	...
	for(i = 0 ; i < TEST_SIZE ; i++) {
		adpcm_data->test_data[i] = (int) j * adpcm_dec_cos(f * PI * i);
...
int adpcm_dec_return()
{
	...
	for(i = 0; i < IN_END; i += 2) {
		check_sum += (adpcm_data->dec_result[i] + adpcm_data->dec_result[i + 1]);
	}
...
void _Pragma("entrypoint") adpcm_dec_main(void)
{
	...
	for(i = 0 ; i < IN_END ; i += 2) {
		adpcm_dec_decode(&adpcm_data->compressed[i / 2]);
		adpcm_data->dec_result[i] = adpcm_dec_xout1;
		adpcm_data->dec_result[i + 1] = adpcm_dec_xout2;
	}
