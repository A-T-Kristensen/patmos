		...
		for j in range(0, len(nbanksList)):         # Iterate over number of banks
			for k in range(0, len(dimList)):        # Iterate over matrix dimensions
				for g in range(0, len(appList)):    # Iterate over apps      
					...
					app = appList[g]
					valsDefine = [dimList[k], dimList[k], dimList[k], nbanksList[j]]                    
...
def filterbank(synth = 0, hw_test = 0):
	...
	appList     = ["hwa_filterbank", "hwa_filterbank_spm", "hwa_filterbank_uncached",
				   "tacle_filterbank", "tacle_filterbank_spm", "tacle_filterbank_uncached"] 
	...
	store_benchmark("filterbank", appList, computeArray, transferArray, totalArray, csv_rows)
